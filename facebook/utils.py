...
def get_long_lived_access_token(api_key, api_secret, access_token):
    url = u"https://graph.facebook.com/v16.0/oauth/access_token?grant_type=fb_exchange_token" \
...
def get_ad_account(token):
    adaccount_dict = {}
    data = []
    adaccount_url = f"https://graph.facebook.com/v16.0/me/adaccounts?fields=id,name&access_token={token}"
...
def single_ad_info(ad_id,token):
    ad_url =  f"https://graph.facebook.com/v16.0/{ad_id}?fields=id,name&access_token={token}"
    ad = fetch(ad_url)
    return (ad['id'], ad['name'])
    

def pages_associated_with_adaccounts(token, ad_account_id="act_296865963"):
    account_url = f"https://graph.facebook.com/v16.0/{ad_account_id}?fields=business&access_token={token}"
    ...
    if "business" in business:
        ...
        pages_url = f"https://graph.facebook.com/v16.0/{business_id}/owned_pages?access_token={token}"
...
def get_adset_name(adset_id="6345796776582", access_token=LONGLIVED_ACCESS_TOKEN):
    adset_url = f"https://graph.facebook.com/v16.0/{adset_id}?fields=id,name&access_token={access_token}"
...
def get_adsets_from_campaign(campaign_id, access_token):
    campaign_ads= f"https://graph.facebook.com/v16.0/{campaign_id}/adsets?fields=id,name,effective_status&access_token={access_token}"
    adsets = json.loads(urllib.request.urlopen(campaign_ads).read()) 
    for d in adsets['data']:
        ads_url = f"https://graph.facebook.com/v16.0/{d['id']}/ads?fields=id,name&access_token={access_token}"
...
def get_ad_id_list(campaign_id ="6338868868582", access_token=LONGLIVED_ACCESS_TOKEN): # all ads from wholesale as there are comments
    campaign_ads= f"https://graph.facebook.com/v16.0/{campaign_id}/ads?fields=id,name,creative.fields(effective_object_story_id),insights.fields(actions)&access_token={access_token}"
...
def get_ad_name_and_effective_object_story_id(ad_id="6338868872182", access_token=LONGLIVED_ACCESS_TOKEN):
    ad_url = f"https://graph.facebook.com/v16.0/{ad_id}?fields=id,name,creative.fields(effective_object_story_id),insights.fields(actions)&access_token={access_token}"
...
def comment_count(eff, access_token):
    comment_url = f"https://graph.facebook.com/v16.0/{eff}/comments?fields=id,toplevel,message&access_token={access_token}"
...
def comment_info(comment_id, access_token=LONGLIVED_ACCESS_TOKEN):
    comment_url = f"https://graph.facebook.com/v16.0/{comment_id}?fields=id,message&access_token={access_token}"
    comment_json = json.loads(urllib.request.urlopen(comment_url).read())
...
def comment_status(comment_id, access_token=LONGLIVED_ACCESS_TOKEN):
    url = f"https://graph.facebook.com/v16.0/{comment_id}?fields=is_hidden,can_hide&access_token={access_token}"
...
def get_account_post_list(account_id, account_access_token):
    url = f"https://graph.facebook.com/v16.0/{account_id}/posts?fileds=id,message&access_token={account_access_token}&limit=100"
    read_url = json.loads(urllib.request.urlopen(url).read())
    post_id = []
