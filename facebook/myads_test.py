...
def save_pages_to_adaccount(request, ad_account_id): #check which page token is valid for the ad accounts 
    ...
    for p in pages: 
        try:
            ...
            campaign_url = f"https://graph.facebook.com/v16.0/{ad_account_id}/campaigns?effective_status=%5B%22ACTIVE%22%2C%22PAUSED%22%5D&fields=name%2Cobjective&access_token={p['access_token']}"
            ...
            if "data" in campaigns_json and len(campaigns_json) != 0:
                ...
                ads_url = f"https://graph.facebook.com/v16.0/{campaign_id}/ads?fields=id,name,insights&access_token={p['access_token']}"
                ...
                if "data" in ads_json and len(ads_json['data']) != 0: 
                    ...
                    eff = f"https://graph.facebook.com/v16.0/{ad_id}?fields=creative.fields(effective_object_story_id),insights.fields(actions)&access_token={p['access_token']}" 
                    ...
                    comments_url = f"https://graph.facebook.com/v16.0/{eff_id}/comments?summary=true&access_token={p['access_token']}"
